...
const HELP_EXAMPLES: Array<[string, string]> = [
  ['image-resize',                              'Analyze current project (no changes)'],
  ['image-resize --folder public',              'Analyze only ./public'],
  ['image-resize --folder public/temp',         'Analyze only ./public/temp'],
  ['image-resize public/',                      'Positional form (same as --folder)'],
  ['image-resize --folder public --apply',      'Resize oversized images under ./public'],
  ['image-resize --apply --delete-unused',      'Resize + delete unused (prompts to confirm)'],
  ['image-resize --size-kb 300',                'Tighten file-size threshold to 300 KB'],
  ['image-resize --max-dimension 1280',         'Tighten longest-edge threshold to 1280 px'],
  ['image-resize --json',                       'JSON output for tooling'],
];

function showHelp(): void {
  console.log('');
  console.log(c.bold('image-resize') + ' — AI-context-safe image analysis and resizing');
  console.log('');
  console.log(c.bold('USAGE'));
  console.log('  image-resize [--folder <path>] [options]');
  console.log('  image-resize [options] [path]              ' + c.dim('# positional form'));
  console.log('');
