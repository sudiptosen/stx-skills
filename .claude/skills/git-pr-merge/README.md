# git-pr-merge Skill — Design Notes

Design rationale and edge-case discussion for the `/git-pr-merge` skill. End-user docs live in [SKILL.md](./SKILL.md).

## Problem statement

`/git-checkin` already covers commit + push + PR creation, but a feature branch's life doesn't end there. After the PR opens, a developer still has to:
...
Each of those steps has a known failure mode, and skipping any of them leaves the repo in a half-cleaned state. `/git-pr-merge` codifies the entire chain so the developer (or Claude) executes them in order and halts on any failure.
...
```
git-pr-merge.ts
├── ANSI Colors (matches git-checkin / image-resize)
├── CLI Options
│   ├── parseArgs()
│   └── showHelp()
├── Git Helpers
│   ├── execGit()                    # shared with the git-checkin shape
│   ├── getCurrentBranch()
│   ├── isMainBranch()
...
- [`/git-checkin` SKILL.md](../git-checkin/SKILL.md) — companion skill for partial flows
