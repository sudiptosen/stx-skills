...
User invokes via `/gen-worktree-report`, optionally with arguments:

```
/gen-worktree-report                                # use cwd, infer everything
/gen-worktree-report --worktree <path>              # explicit worktree path
/gen-worktree-report --base <branch>                # diff base (default: main)
/gen-worktree-report --output <path.html>           # explicit output file
/gen-worktree-report --title "<one-line title>"     # override report title
```

The CLI helper `gen-worktree-report` (when run from a terminal) prints worktree context as JSON. When invoked as a slash command, follow the procedure below.

## Procedure (when invoked as `/gen-worktree-report`)
